...
class OxSetupDb extends oxSetupCore
{
    ...
    public function execSql($sQ)
    {
        ...
        if ($rReturn === false) {
            throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_BAD_SQL') . "( $sQ ): " . $this->_oConn->connect_error . "\n");
        }
    ...
    public function testCreateView()
    {
        ...
        $rReturn = mysqli_query($this->getConnection(), $sQ);
        if ($rReturn === false) {
            throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_VIEWS_CANT_CREATE') . " " . $this->_oConn->connect_error . "\n");
        }
        ...
        $rReturn = mysqli_query($this->getConnection(), $sQ);
        if ($rReturn === false) {
            throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_VIEWS_CANT_SELECT') . " " .  $this->_oConn->connect_error . "\n");
        }
        ...
        $rReturn = mysqli_query($this->getConnection(), $sQ);
        if ($rReturn === false) {
            throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_VIEWS_CANT_DROP') . " " .  $this->_oConn->connect_error . "\n");
        }
    }
    ...
    public function openDatabase($aParams)
    {
        ...
        if ($this->_oConn === null) {
            // ok open DB
            $this->_oConn = @mysqli_connect($aParams['dbHost'], $aParams['dbUser'], $aParams['dbPwd'], $aParams['dbName'] );
            if (!$this->_oConn) {
                $oSetup = $this->getInstance("oxSetup");
                $oSetup->setNextStep($oSetup->getStep('STEP_DB_INFO'));
                throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_DB_CONNECT') . " - " . $this->_oConn->connect_error, oxSetupDb::ERROR_DB_CONNECT);
            }
            ...
            if (!(@mysqli_select_db($aParams['dbName'], $this->_oConn))) {
                throw new Exception($this->getInstance("oxSetupLang")->getText('ERROR_COULD_NOT_CREATE_DB') . " - " . $this->_oConn->connect_error, oxSetupDb::ERROR_COULD_NOT_CREATE_DB);
            }
        }
    ...
    public function createDb($sDbName)
    {
        if (!$this->execSql("create database `" . $sDbName . "`")) {
            ...
            $oSetup = $this->getInstance("oxSetup");
            $oSetup->setNextStep($oSetup->getStep('STEP_DB_INFO'));
            throw new Exception(sprintf($this->getInstance("oxSetupLang")->getText('ERROR_COULD_NOT_CREATE_DB'), $sDbName) . " - " . $this->_oConn->connect_error);
        }
    }
    ...
    public function dbConnect()
    {
        ...
        try {
            ...
            if ($oExcp->getCode() === oxSetupDb::ERROR_DB_CONNECT) {
                $oSetup->setNextStep($oSetup->getStep('STEP_DB_INFO'));
                $oView->setMessage($oLang->getText('ERROR_DB_CONNECT') . " - " . $this->_oConn->connect_error);
